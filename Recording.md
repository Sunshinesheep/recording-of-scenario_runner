...
完成对行人的set_position()函数的编写，并通过修改`osc2_scenario_configuration.py`的visit_modifier_invocation()函数模块，使其支持对行人相关函数的运行。
...
另外，编写函数set_color()，实现车辆模型颜色可调：

设置为蓝色

![](/home/lhy/recording-of-scenario_runner/IMG/blue.png)
...
关于Path的issue，项目中确实存在该函数，但是其作用仅仅为接受该参数，并没有对场景的设置有所影响。也查阅了Carla的文档，也并没有相关的API可以使用。



关于地图的导入问题，目前通用的方法是先使用Roadrunner'构建三维地图，再通过Carla提供的命令进行导入。从OSC2文件中导入，可能较为困难。
